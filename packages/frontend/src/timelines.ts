...
export function isAvailableBasicTimeline(timeline: BasicTimelineType | undefined | null): boolean {
	switch (timeline) {
		case 'home':
			return $i != null;
		case 'local':
			return ($i == null && instance.policies.ltlAvailable) || ($i != null && $i.policies.ltlAvailable && !$i.makeyHideLocalTimeline);
		case 'social':
			return ($i != null && $i.policies.ltlAvailable && !$i.makeyHideLocalTimeline);
		case 'global':
			return ($i == null && instance.policies.gtlAvailable) || ($i != null && $i.policies.gtlAvailable && !$i.makeyHideFederatedTimeline);
		default:
			return false;
	}
}
